...
- **The JS playground** — `playground/`, built by `npm run playground` into `playground/dist`: JavaScript on one side, a notebook over it on the other, the engine, the scanner, the reader and part of the JS model running in the browser. A formal cell is written in sentences, as a `.rofl.md` is, and a rofl cell in plain Datalog; `? S` lists, `never S` is an invariant, `why S` explains; a natural cell is translated by Claude when the page runs as an artifact.
