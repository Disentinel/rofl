...
- **The NPC yard, live** — `playground/npc.html`, built with the playground: `examples/npc` run one tick at a time in the browser, each agent's knowledge, intents, options and chosen act on the page, `why` and `whynot` on any of them at any kept tick, and the rule the agents propose from their holes, taken and applied to the past. The yard's core is `examples/npc/sim.ts`, which `demo.ts` also runs; its sentences are `examples/npc/phrases.rofl`.
