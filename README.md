...
- **The JS playground** — `playground/`, built by `npm run playground` into `playground/dist`: JavaScript on one side, a notebook of ROFL cells over it on the other, the engine, the scanner and part of the JS model running in the browser. `? L` lists, `never L` is an invariant, `why L` explains; a natural cell is translated by Claude when the page runs as an artifact.
