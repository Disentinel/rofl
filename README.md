...
- **Ring 0, in its own words** — `docs/rings/`: the kernel's four programs of rules (`boot.rofl`, `safety.rofl`, `policy.rofl`, `rules/strata.rofl`) rendered as sentences by `npm run render:rings`, phrases in `facts/kernel-phrases.rofl`; each reads back into its source clause for clause. `docs/rings/README.md` says what is there and what is not.
