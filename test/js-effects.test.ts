...
test('the effect layer answers 100 js cells and the partition closes', () => {
  ...
  assert.deepEqual(w.binds('verdict[audit](js, K, S, effect, not_modelled)').map((x) => x.split('/')[0]).sort(), [
    // FIVE LEFT THIS LIST on 2026-09-09 (w_effect_class_initialisers):
    // `class_declaration`, `class_property`, `class_private_property`,
    // `class_accessor_property` and `static_block` — the kinds that run code at
    // a moment other than a call.
    'binary_expression',
    'call_expression',
    'debugger_statement',
