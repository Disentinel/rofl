...
test('the control-flow layer costs a number, and it is nearly half of its world', () => {
  ...
  // +3 716 rows and +218 firings on 2026-09-09, ALL OF IT IN THIS PACK — the
  // world and the layer move by exactly the same amount, which is what a
  // difference-based attribution looks like when the change really is local.
  // `w_destructuring_hides_a_call`'s second pass added the `hidden_call_*`
  // census: seven relations over 28 positions that answer *which of the two
  // opposite things does this rule's silence mean*. 218 derivations for the
  // denominator of four cells that were closed without one.
  assert.equal(L.world.total, 3069495, 'rows handed out by the store in the control-flow fixpoint');
  assert.equal(L.world.firings, 171497, 'derivations in the control-flow fixpoint');
