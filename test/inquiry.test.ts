...
test('ambiguous claim produces clarify, and never verify', () => {
  const r = fixture();
  assert.ok(r.holds('candidate_intent(clarify, atlas_launch, legacy_import_required)'));
  assert.ok(!r.holds('candidate_intent(verify, atlas_launch, legacy_import_required)'));
});
...
test('supported + refuted produce contested, which produces discriminate', () => {
  ...
  assert.ok(r.holds('candidate_intent(discriminate, atlas_launch, delivery_rate_ok)'));
});

test('authority-only claim produces escalate', () => {
  const r = fixture();
  assert.ok(r.holds('candidate_intent(escalate, atlas_launch, capacity_risk_accepted)'));
});

test('resolved obligations produce no intent at all', () => {
  const r = fixture();
  for (const c of ['requirements_complete', 'monitoring_ready', 'rollback_ready']) {
    assert.ok(r.holds(`resolved_obligation(atlas_launch, ${c})`), `${c} resolved`);
    assert.ok(!r.holds(`candidate_intent(K, atlas_launch, ${c})`), `${c} quiet`);
  }
});
...
test('refuted blocking claim derives no_go, explainable to the evidence', () => {
  ...
  const why = r.why('recommendation(atlas_launch, no_go)');
  assert.ok(why.ok);
  assert.match(why.text, /violated_blocking/);
  assert.match(why.text, /chat_note_17/, 'traces to the journal entry');
});

test('why explains where a verify intent came from', () => {
  const r = fixture();
  const why = r.why('candidate_intent(verify, atlas_launch, aggregate_capacity_verified)');
  assert.ok(why.ok);
...
test('a contested blocking claim blocks GO even while supported', () => {
  const r = fixture();
  r.assert('blocking(atlas_launch, delivery_rate_ok).');
  assert.ok(r.holds('supported[epistemic](delivery_rate_ok)'));
  assert.ok(r.holds('go_blocked(atlas_launch, delivery_rate_ok)'));
});
...
test('mutation: new evidence closes the obligation and retires the intent', () => {
  ...
  assert.ok(!r.holds('open_obligation(atlas_launch, aggregate_capacity_verified)'));
  assert.ok(!r.holds('candidate_intent(verify, atlas_launch, aggregate_capacity_verified)'));
...
test('excise of the refuting evidence: the diff is the verdict blast radius', () => {
  const r = fixture();
  const e = r.excise('refutes[obs](chat_note_17, billing_e2e_verified)');
  assert.ok(e.ok);
  assert.ok(e.removed.includes('recommendation[main](atlas_launch,no_go)'));
  assert.ok(e.removed.includes('refuted[epistemic](billing_e2e_verified)'));
  assert.ok(e.added.some((k) => k.includes('candidate_intent[main](verify,atlas_launch,billing_e2e_verified')),
...
test('the epistemic report renders the full picture after any state', () => {
  ...
  assert.match(report, /atlas_launch \(decide\)/);
  ...
  assert.match(report, /clarify: legacy_import_required/);
  assert.match(report, /escalate: capacity_risk_accepted/);
  assert.match(report, /chat_note_17/, 'the why section traces to evidence');
});
