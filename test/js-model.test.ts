...
const binds = (r: Rofl, q: string, v: string): string[] => {
  const res = r.query(q);
  assert.equal(res.unpopulatable, false, `query ${q}: nothing in this world can populate it`);
  return res.rows.map((x) => String(x.bindings[v]).replace(/^"|"$/g, '')).sort();
};
...
test('the shape axis loads, every kernel audit is empty, and the paper prediction holds', () => {
  ...
  assert.equal(n(r, 'node_kind(A, B)'), 66, '61 js + 5 py');
  assert.equal(n(r, 'layer(L)'), 2);
  ...
  assert.equal(coarse.cell, 132, 'predicted 132 coarse cells');
  ...
  assert.deepEqual(f, { cell: 156, modelled: 56, waived: 26, not_modelled: 74 },
    'predicted 156 fine cells = 56 + 26 + 74');
...
test('the three verdicts still partition the cell space with a third axis', () => {
  ...
  assert.equal(n(r, 'reason[audit](A, B, S, L, R)'), 74, 'one reason per not_modelled cell');
  ...
  assert.equal(n(r, 'irreducible_unknown[audit](A, B, S, L)')
             + n(r, 'our_unknown[audit](A, B, S, L)'), 74, 'the split is a work queue');
...
test('every declared kind either appears in the corpus or says why not', () => {
  ...
  assert.equal(n(r, 'kind_absent_ok(K, R)'), 5, 'positive control: five rows reach this world');
...
test('MUTANT: an excuse withdrawn names the kind, and a misspelled one names it twice', () => {
  ...
  const gone = corpus({
    cg: CG.replace('kind_absent_ok(tagged_template_expression, a_no_corpus_site).', ''),
  });
  assert.deepEqual(binds(gone, 'kind_unexercised[audit](A, K)', 'K'), ['tagged_template_expression'],
    'a declared kind with no node and no reason is named');

  const typo = corpus({
    cg: CG.replace('kind_absent_ok(decorator, a_no_corpus_site).',
                   'kind_absent_ok(decorater, a_no_corpus_site).'),
  });
  assert.deepEqual(binds(typo, 'kind_unexercised[audit](A, K)', 'K'), ['decorator'],
    'the kind is unexcused again');
  assert.deepEqual(binds(typo, 'kind_absent_undeclared[audit](K)', 'K'), ['decorater'],
...
test('the declared shapes agree with the census the rules produce on the corpus', () => {
  ...
  assert.equal(sites, 360, 'positive control: the corpus is the one the census was taken on');
...
test('the shape verdicts for member_expression match what the runtime missed', async () => {
  ...
  assert.equal(oracleEdges.size, 157, 'the oracle saw the call graph docs/modelling-a-language.md records');
...
test('SHAPE MUTANT 1: removing axis_applies collapses the matrix onto the coarse one', () => {
  ...
  assert.equal(f.cell, 132, 'predicted 132');
  ...
  assert.equal(f.modelled, 62, '32 kind-level claims + 30 shape claims with no cell under them');
...
test('SHAPE MUTANT 2: declaring the shape axis applicable to `modules` is refused', () => {
  ...
  assert.equal(counts(r).cell, 198, 'predicted 198 coarse');
  assert.equal(f.cell, 222, '156 + 66 unrefined none-cells, and NOT one shaped cell');
  assert.equal(f.cell - before.cell, 66);
...
test('SHAPE MUTANT 2b: the same refusal for `dataflow`, which the first draft declared', () => {
  ...
  assert.equal(f.cell, 156, 'not one cell moves: no shape declares itself in dataflow');
  assert.deepEqual(r.query('unearned_axis[audit](A, L)').rows
...
test('SHAPE MUTANT 3: a shape declared for a kind nobody declared', () => {
  ...
  assert.equal(f.cell, 157, 'predicted 156 + 1: the ghost gets a cell of its own');
...
test('SHAPE MUTANT 4: a split kind that also keeps its unrefined cell', () => {
  ...
  assert.equal(f.cell, 172, 'predicted 156 + 16, one per splitting kind');
...
test('SHAPE MUTANT 5a: dropping the unsplit-`none` branch deletes 26 kinds from callgraph', () => {
  ...
  assert.equal(f.cell, 106, 'predicted 156 - 50');
  assert.equal(n(r, 'lost_cell[audit](A, B, L)'), 50, 'a coarse cell with nothing under it');
...
test('SHAPE MUTANT 5b: dropping the not-applicable-`none` branch deletes a whole layer', () => {
  ...
  assert.equal(f.cell, 90, 'predicted 156 - 66: the dataflow layer disappears');
  assert.equal(n(r, 'lost_cell[audit](A, B, L)'), 66);
...
test('SHAPE MUTANT 6: member_expression left with one shape instead of twenty-one', () => {
  ...
  assert.equal(removed, 20, 'positive control: twenty rows were removed, and one was kept');

  const r = shapeWorld({ shapes });
  assert.equal(n(r, 'shape_of(js, member_expression, S)'), 1);
  const f = fine(r);
  showFine('mutant 6 (one shape for member_expression)', f);
  assert.equal(f.cell, 136, 'predicted 156 - 20');
