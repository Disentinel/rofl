...
for (const m of vocab.matchAll(/^sig\((\w+), "([^"]+)"\)/gm)) {
  ...
  args.forEach((a, k) => { const toks = a.split(/\s+/); const last = toks.pop()!; const pos = last.includes(':') ? Number(last.split(':')[1]) : k; const noun = toks.filter((t) => !MARKERS.has(t)).join(' '); if (noun) signed.push(`signed(${m[1]}, ${pos}, ${JSON.stringify(noun)}).`); });
}
for (const m of vocab.matchAll(/^phrase\((\w+), "([^"]+)"\)/gm)) {
  let k = 0;
  let top = -1;
  for (const h of m[2].matchAll(/<([^>]+)>/g)) { const inner = h[1]; if (inner.includes('=')) { top = Math.max(top, Number(inner.split('=')[0])); continue; } const [a, b] = inner.includes(':') ? inner.split(':') : [String(k++), inner]; top = Math.max(top, Number(a)); signed.push(`signed(${m[1]}, ${Number(a)}, ${JSON.stringify(b.trim())}).`); }
  sigArity.set(m[1], [...(sigArity.get(m[1]) ?? []), top + 1]);
}
const twoArities: string[] = [];
for (const [rel, ars] of arities) if (ars.size > 1 && sigArity.has(rel)) twoArities.push(`two_arities(${rel}, ${[...ars].sort().join(', ')}).`);
const signedRels = [...sigArity.keys()].map((r) => `signed_rel(${r}).`);
...
if (twoArities.length) console.log('a name with two arities and one signature: ' + twoArities.map((t) => t.slice(12, -2)).join('; '));
const bySigned = new Map<string, Set<string>>(); for (const b of inSigned) { const s = bySigned.get(String(b.Rel)) ?? new Set(); s.add(clean(b.V)); bySigned.set(String(b.Rel), s); }
...
for (const [rel, vs] of [...bySigned].sort((a, b) => b[1].size - a[1].size).slice(0, 25)) {
  ...
  console.log(`  ${rel.padEnd(24)} ${[...vs].sort().join(' ').padEnd(10)} ${line ? line[1].replace(/\[([^\]]*)\]\([^)]*\)/g, '$1').slice(0, 110) : ''}`);
}
