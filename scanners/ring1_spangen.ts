...
function main(): void {
  const ex = extract();
  const { rels, refused, valued, projected } = ex;
  const outPath = (() => {
    const argv = process.argv.slice(2);
    for (let i = 0; i < argv.length; i++) if (argv[i] === '--out') return argv[++i]!;
    return path.join(REPO, 'rust/rofl/src/ring1_tables.rs');
  })();
  fs.writeFileSync(outPath, emitTables(ex));
...
// ---------------------------------------------------------------------------
// RENDERING: generated TABLES plus a driver written once, which is what a
// parser generator does for an LR machine. The tables come from the rules and
// nothing else; the driver is fixed and is the Rust twin of `evaluate` above,
// which is validated by set equality against ring1.
//
// Generating control flow instead was the alternative and it is the riskier
// one: six offset-and-frame mistakes were made reading these rules today, and
// every one of them would have become a wrong-but-compiling Rust function.
// A table is inspectable; a generated loop is not.

const rustStr = (s: string): string => JSON.stringify(s);

function testRust(t: Test): string {
  switch (t.t) {
    case 'kind': return `T::Kind(${t.off}, ${rustStr(t.kind)})`;
    case 'char': return `T::Char(${t.off}, ${rustStr(t.ch)})`;
    case 'state': return `T::State(${t.off}, ${rustStr(t.state)})`;
    case 'pred': return `T::Pred(${t.off}, ${rustStr(t.rel)})`;
    case 'not': return `T::Not(&${testRust(t.x)})`;
  }
}
function stepRust(s: Step): string {
  switch (s.k) {
    case 'test': return `S::Test(${s.frame}, ${testRust(s.test)})`;
    case 'consume': return `S::Consume(${rustStr(s.rel)})`;
    case 'search': return `S::Search(${testRust(s.test)})`;
    case 'valued': return `S::Valued(${rustStr(s.rel)})`;
    case 'projected': return `S::Test(0, ${testRust({ t: 'pred', off: 0, rel: s.rel })})`;
  }
}

export function emitTables(ex: ReturnType<typeof extract>): string {
  const L: string[] = [];
  L.push('// GENERATED by scanners/ring1_spangen.ts from examples/ring1/ring1.rofl.');
  L.push('// Do not edit: edit the RULES and re-run the generator. These are TABLES;');
  L.push('// the driver that walks them is `ring1_tok.rs` and is written once.');
  L.push('// The oracle is ring1 itself — test/ring1-spangen.test.ts asserts that the');
  L.push('// set of token spans equals the set the rules derive.');
  L.push('use crate::ring1_tok::{Prod, Rel, Step as S, Test as T, Valued};');
  L.push('');
  L.push(`pub const RELS: &[Rel] = &[`);
  for (const r of ex.rels) {
    L.push(`    Rel { name: ${rustStr(r.rel)}, prods: &[`);
    for (const p of r.prods) {
      if (p.p === 'while') {
        L.push(`        Prod::While { step: &[${p.step.map(testRust).join(', ')}] },`);
      } else {
        L.push(`        Prod::Chain { steps: &[${p.steps.map(stepRust).join(', ')}], `
          + `end_off: ${p.endOff === null ? 'None' : `Some(${p.endOff})`} },`);
      }
    }
    L.push('    ] },');
  }
  L.push('];');
  L.push('');
  L.push('pub const VALUED: &[Valued] = &[');
  for (const v of ex.valued) {
    L.push(`    Valued { name: ${rustStr(v.rel)}, cases: &[`);
    for (const c of v.cases) L.push(`        (&[${c.tests.map(testRust).join(', ')}], ${rustStr(c.value)}),`);
    L.push('    ] },');
  }
  L.push('];');
  L.push('');
  L.push('/// `(projection, span it projects, true when onto the END)`');
  L.push('pub const PROJECTED: &[(&str, &str, bool)] = &[');
  for (const p of ex.projected) L.push(`    (${rustStr(p.rel)}, ${rustStr(p.of)}, ${p.end}),`);
  L.push('];');
  return L.join('\n') + '\n';
}
