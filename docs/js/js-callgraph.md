...
*await*, *call*, *class*, *declarator*, *decorator*, *field*, *file*, *function*, *index*, *key*, *kind*, *line*, *member access*, *method*, *name*, *node*, *object literal*, *object method*, *property*, *super*, *template*.
...
<a id="call_site"></a>`call_site`(a node C, File) if C [is of kind](js-model.md#ast_node) K in file File and [`call_kind`](#call_kind)(K).

<a id="call_site_kind"></a>`call_site_kind`(a node C, K) if C [is of kind](js-model.md#ast_node) K and [`call_kind`](#call_kind)(K).

<a id="call_line"></a>`call_line`(a node C, Line) if [`call_site`](#call_site)(C, something) and C [is of kind](js-model.md#ast_node) some kind in file some file at line Line.

<a id="callee_of"></a>`callee_of`(a node C, a node N) if [`call_site`](#call_site)(C, something) and the callee of C is N.

<a id="callee_kind"></a>`callee_kind`(C, K) if [`callee_of`](#callee_of)(C, a node N) and N [is of kind](js-model.md#ast_node) K.
...
<a id="transfer_site"></a>`transfer_site`(a node X, K) if X [is of kind](js-model.md#ast_node) K and [`transfer_kind`](#transfer_kind)(K).
...
<a id="call_arg"></a>`call_arg`(a node C, I, a node A) if [`call_site`](#call_site)(C, something) and A is the I-th of the arguments of C.
...
<a id="member_like"></a>`member_like`(C, a node N) if all of:
...
<a id="computed_member"></a>`computed_member`(C, a node N) if [`member_like`](#member_like)(C, N) and the attribute computed of N is true.

<a id="static_member"></a>`static_member`(C, a node N) if [`member_like`](#member_like)(C, N) and the attribute computed of N is false.

<a id="callee_obj"></a>`callee_obj`(C, a node O) if [`member_like`](#member_like)(C, a node N) and the object of N is O.

<a id="callee_obj_kind"></a>`callee_obj_kind`(C, K) if [`callee_obj`](#callee_obj)(C, a node O) and O [is of kind](js-model.md#ast_node) K.

<a id="callee_prop"></a>`callee_prop`(C, a node P) if [`member_like`](#member_like)(C, a node N) and the property of N is P.
...
- [`callee_prop`](#callee_prop)(C, a node P);
...
<a id="multi_shape"></a>`multi_shape`(C, A, B) if [`shape`](#shape)(C, A), [`shape`](#shape)(C, B), and A differs from B.
...
<a id="fn_node"></a>`fn_node`(a node F) if F [is of kind](js-model.md#ast_node) K and [`fn_kind`](#fn_kind)(K).

<a id="fn_file"></a>`fn_file`(a node F, File) if [`fn_node`](#fn_node)(F) and F [is of kind](js-model.md#ast_node) some kind in file File.
...
<a id="fn_name"></a>`fn_name`(a node F, N) either:

1. if F is a function, the id of F is a node I, and I [is named](js-structure.md#ast_name) N;
2. if F is a function, the id of F is a node I, and I [is named](js-structure.md#ast_name) N;
3. if F is an object method, the key of F is a node K, and K [spells](js-structure.md#key_name) N.

<a id="ctor_method"></a>`ctor_method`(a method F) if the attribute kind of F is "constructor".

`fn_name`(a node F, N) either:

1. if all of:
   - F is a method;
   - the key of F is a node K;
...
<a id="decorates"></a>A node Owner is decorated by a decorator D if D is among the decorators of Owner.

<a id="in_own_decorator"></a>`in_own_decorator`(a node F, a node C) either:

1. if F [is decorated by](#decorates) a node D and D [is within](js-structure.md#ast_within) C;
2. if F [is decorated by](#decorates) C.

<a id="encloses"></a>`encloses`(a node F, a node C) if all of:
- [`fn_node`](#fn_node)(F);
- F [is within](js-structure.md#ast_within) C;
- [`site`](#site)(C);
- unless [`in_own_decorator`](#in_own_decorator)(F, C).

<a id="closer"></a>`closer`(a node F, C) if all of:
- [`encloses`](#encloses)(F, C);
- [`encloses`](#encloses)(a node G, C);
...
<a id="top_call"></a>`top_call`(a node C, R) if all of:
- [`site`](#site)(C);
...
<a id="obj_member_fn"></a>`obj_member_fn`(a node O, Key, a node M) either:
...
<a id="class_ctor"></a>`class_ctor`(a class CD, a method M) if CD [has the method](js-dataflow.md#class_method_of) M and the attribute kind of M is "constructor".

<a id="resolves"></a>A call X resolves to a function M either:
...
A call X resolves to a function M if [`for_of_iterates`](#for_of_iterates)(X, M).

<a id="calls"></a>`calls`(Caller, a node Next) if all of:
...
A call C resolves to a function F if all of:
- [`callee_of`](#callee_of)(C, a node N);
- N [may be the node](js-dataflow.md#may_be_node) F;
- [`fn_node`](#fn_node)(F).

<a id="ambiguous_call"></a>`ambiguous_call`(a call C, a function F, a function G) if C [resolves to](#resolves) F, C [resolves to](#resolves) G, and F differs from G.
...
`calls`(Caller, a function Callee) either:

1. if a call C [resolves to](#resolves) Callee and [`nearest_fn`](#nearest_fn)(Caller, C);
2. if a call C [resolves to](#resolves) Callee and [`top_call`](#top_call)(C, Caller).

<a id="calls_named"></a>`calls_named`(A, B) either:

1. if [`calls`](#calls)(X, Y), [`fn_name`](#fn_name)(X, A), and [`fn_name`](#fn_name)(Y, B);
2. if [`calls`](#calls)(R, Y), `ast_file`(R, something), [`fn_name`](#fn_name)(Y, B), and A is top.

<a id="calls_in"></a>`calls_in`(File, A, B) either:

1. if all of:
   - [`calls`](#calls)(a node X, Y);
...
<a id="resolved_site"></a>`resolved_site`(a call C) if C [resolves to](#resolves) some function.
...
1. if [`unresolved_call`](#unresolved_call)(a node C, S) and C [is of kind](js-model.md#ast_node) some kind in file File at line Line;
2. if all of:
   - [`transfer_site`](#transfer_site)(a node X, S);
...
<a id="shape_verdict"></a>`shape_verdict`(S, absent) either:
...
<a id="fn_binder"></a>`fn_binder`(a node D, a node F) either:
...
4. if [`callee_prop`](#callee_prop)(something, a node P) and P [is of kind](js-model.md#ast_node) K;
5. if [`fn_node`](#fn_node)(a node F) and F [is of kind](js-model.md#ast_node) K;
6. if [`fn_binder`](#fn_binder)(a node B, something) and B [is of kind](js-model.md#ast_node) K.
...
<a id="await_arg"></a>`await_arg`(an await A, a node X) if the argument of A is X.
...
<a id="performed_call"></a>`performed_call`(A, F) if [`awaited_then`](#awaited_then)(A, F).
...
<a id="await_value_known"></a>`await_value_known`(A) if [`await_arg`](#await_arg)(A, a node X) and X [may be the node](js-dataflow.md#may_be_node) some node.

<a id="await_no_call"></a>`await_no_call`(A) if [`await_value_known`](#await_value_known)(A), unless [`awaited_then`](#awaited_then)(A, something).

<a id="await_value_unknown"></a>`await_value_unknown`(A) if [`await_arg`](#await_arg)(A, something), unless [`await_value_known`](#await_value_known)(A).
...
<a id="awaiting_fn"></a>`awaiting_fn`(a node Fn, a node A) if all of:
- [`performed_call`](#performed_call)(A, something);
- [`fn_node`](#fn_node)(Fn);
- Fn [is within](js-structure.md#ast_within) A.

<a id="performed_as_edge"></a>`performed_as_edge`(Fn, F) if all of:
- [`performed_call`](#performed_call)(A, F);
- [`awaiting_fn`](#awaiting_fn)(Fn, A);
- [`calls`](#calls)(Fn, F).
