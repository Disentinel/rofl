...
async function week(): Promise<Group> {
  ...
  // PLANTED (3): no week_starts for the date — the loader mints the week from the date, whoami says so once, an edit lands in
  // the minted week, the world book is not written; with the world's own line (w0907) there is no «заведена по дате»
  const n = await spat(root, 'robin', ['whoami'], { SPAT_NOW: '2026-09-14T09:00:00+03:00' });
  g.check('пн 14.09 (в мире нет week_starts): whoami → 0 «… · неделя w0914 (заведена по дате) · сегодня 2026-09-14»', n.code === 0 && /· неделя w0914 \(заведена по дате\) · сегодня 2026-09-14$/m.test(n.out), n.out);
  const t = one(robin(root, '2026-09-16T09:00:00+03:00'), 'edit', ['add t mon 10:00-11:00 kit']);
  g.check('ср 16.09: add t mon … → 0 «(2026-09-21) … — неделя w0921» (следующий понедельник, неделя заведена по дате); for_week w0921; в книге world нет week_starts(w0921)', codeOf(t) === '0' && /\(2026-09-21\) \(e_[0-9a-f]+\) — неделя w0921/.test(t) && weekOfEdit(root, idOf(t)) === 'w0921' && sql<{ n: number }[]>(root, "SELECT count(*) n FROM facts WHERE ledger = 'world' AND pred = 'week_starts' AND args LIKE '%w0921%'")[0].n === 0, t);
  g.check('пн 21.09: show mon → «пн 2026-09-21 (неделя w0921)», в нём t [правка]; need list → 0; --week-of w0907 → 0', /^пн 2026-09-21 \(неделя w0921\)/.test(one(robin(root, '2026-09-21T09:00:00+03:00'), 'show', ['mon'])) && /  t +/.test(one(robin(root, '2026-09-21T09:00:00+03:00'), 'show', ['mon'])) && codeOf(one(robin(root, '2026-09-14T09:00:00+03:00'), 'need', ['list'])) === '0'
    && (await spat(root, 'robin', ['show', 'mon', '--week-of', 'w0907'], { SPAT_NOW: '2026-09-14T09:00:00+03:00' })).code === 0);
  g.check('мир с явной week_starts (w0907, вт 08.09): whoami без «заведена по дате»; строка мира — приоритет, второй недели w0907 нет', !/заведена по дате/.test(w.out) && one(robin(root, '2026-09-08T10:00:00+03:00'), 'whoami', []).split('\n').filter((l) => /заведена/.test(l)).length === 0);
