...
| `TerrasAlmostAll.lean` | **The culmination** (self-contained, ~1400 lines): merges the two chains above and adds the GENERAL counting theorem `NN_eq_dpf`/`NU_eq_uf` (undecided residues of the real map = string DP, ALL k — proof pairs the two lifts r, r+2^j via `lift_flip`); **`terras_almost_all`** (classes + drop); `terras_integers` / `terras_integers_log` (integer counting, sharp k ≥ 25m threshold); `density_decay_log` / `eta_exponential` (η_k ≤ 2^−⌊k/25⌋); **`collatz_original_integers`** (the plain 3n+1 \| n/2 map via the T^i = C^j, i ≤ j ≤ 2i correspondence); `never_dropper_in_core` (potential descent counterexamples are undecided at every depth ≤ log₃ n). `#print axioms`: propext, Classical.choice, Quot.sound throughout. |
